...
bool parseESPData(byte data[], int ready_pin)
{
  ...
  else if (SPIMsg[0] >> 5 == 0b100)
  { // compare the first 3 bits
    state = GOTO_TARGET;

    DEBUG_SERIAL.println("Starting move to target position");
  }
  else if (SPIMsg[0] >> 5 == 0b101)
  { // compare the first 3 bits
...
ISR(SPI_STC_vect)
{
  ...
  else if (SPIBytesReciev == 2)
  { // one command contains 2 bytes
    SPIBytesReciev = 0;
